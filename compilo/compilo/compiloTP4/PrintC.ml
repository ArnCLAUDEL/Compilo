...
let rec  print_expression o = function
  ...
  | BOperator(e1,SetReference,e2) ->
     fprintf o "(*%a=%a)" print_expression e1
                    print_expression e2
  | BOperator(e1,b,e2) ->
     fprintf o "%a%a%a" print_expression_par e1
                    print_bop b
...
and print_statement tab o = function
  ...
  | IfStat(e,s1,s2) ->
     fprintf o "%aif(%a)\n%a%aelse\n%a" print_tab tab
                    print_expression e
                    (print_statement_curly (tab+2)) s1 print_tab tab
                    (print_statement_curly (tab+2)) s2
...
and print_statement_curly tab o = function
  | BlockStat(_) as s -> print_statement tab o s
  | s ->  fprintf o "%a{\n" print_tab tab;
