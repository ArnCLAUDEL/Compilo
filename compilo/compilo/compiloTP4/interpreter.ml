...
let clear_bits i =
  i land ((lnot (prog_bit lor stack_bit lor heap_bit)))
let unbox_prog i =
  if i land prog_bit = 0 then raise Invalid_jmp
  else clear_bits i
let box_prog i =
  i lor prog_bit
let unbox_heap i =
  if i land heap_bit = 0 then raise Invalid_heap_addr
  else clear_bits i
let box_heap i =
  i lor heap_bit
let unbox_stack i =
  if i land stack_bit = 0 then raise Invalid_stack_addr
  else clear_bits i
...
let print_val i =
  if i land prog_bit <> 0 then
    ...
    print_color (string_of_int i) 34
  else print_int i
      
let get_reg m r =
  m.register.(reg_to_mem r)
let set_reg m r v =
  m.register.(reg_to_mem r) <- v
...
let print_register m =
  ...
  pe ();
  print_string "[0m\n";
  Array.iteri (fun i x -> if (b || i<10 || i=16) then ( 
    print_val x;
    print_string "\t")) m.register;
  print_flag m; print_string "\t\n";
...
let ifFindVar v sl t f =
  try match StringMap.find v sl with
    VarId(vn) -> t vn | _ -> f
  with Not_found -> f
let ifFindLabel l sl t f =
  try match StringMap.find l sl with
    ProgCount(ln) -> t ln | _ -> f
  with Not_found -> f
...
let link main il =
  let lablist,_,globvarcount =
    List.fold_left (fun (ll,ic,vc) i ->
      match i with
      | Label(L l) -> (StringMap.add l (ProgCount ic) ll,ic+1,vc)
      | GlobVar(l,None) -> (StringMap.add l (VarId vc) ll,ic+1,vc+1)
      | GlobVar(l,Some i) -> (StringMap.add l (VarId vc) ll,ic+1,vc+1)
      | _ -> (ll,ic+1,vc)) (specialfun (),0,0) il in
  let il2,_,lablist2 = List.fold_left (fun (il,ic,sm) i -> match i with 
      NOP | Label(_) | LinkInstr(_) | GlobVar(_) ->
	(il,ic,(StringMap.map (function ProgCount(x) when x > ic -> ProgCount(x-1) | x->x) sm))
    | x -> x::il,ic+1,sm
  ) ([],0,lablist) il
  in
  let la = function
    | L(l) -> ifFindLabel l lablist2 (fun li -> C (IL(box_prog li))) (L l)
    | ARL(l,RIP) when StringMap.mem l lablist2 -> (match StringMap.find l lablist2 with
      | VarId(vi) -> AC (IL (box_heap vi))
      | ProgCount(ci) -> AC ((IL (box_prog ci))))
    | ARL(l,r) as x-> ifFindVar l lablist2 (fun vi -> ARI ((IL (box_heap vi)),r)) x
    | x -> x in
  let il3 =
    il2 |> List.rev
  |> List.map (map_arg la) in
  (Array.of_list il3,
   (match StringMap.find main lablist2 with ProgCount x -> x | _ -> failwith "No main"))
...
let eval_arg m = function
  | C(IL i) -> i
  | R(r) -> get_reg m r
  | ARI(IL i,r) -> let rad = get_reg m r in
		if rad land stack_bit <> 0 then
		  m.stack.(i+ (unbox_stack rad))
...
let run_one_step m =
  ...
  | ADD (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v1+v2) a2
  | SUB (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v2-v1) a2
  | MUL (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v2*v1) a2
  | DIV (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v2/v1) a2
  | AND (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v2 land v1) a2
  | XOR (a1,a2) -> let v2 = eval_arg m a2 in
		   let v1 = eval_arg m a1 in
		   store_arg m (v2 lxor v1) a2
  | OR (a1,a2) -> let v2 = eval_arg m a2 in
		  let v1 = eval_arg m a1 in
		  store_arg m (v2 lor v1) a2
  | NOT a ->   let v = eval_arg m a in
	       store_arg m (lnot v) a
  | NEG a ->   let v = eval_arg m a in
	       store_arg m (if v=0 then 1 else 0) a
  | TEST (a1,a2) -> let v2 = eval_arg m a2 in
		    let v1 = eval_arg m a1 in
		    m.zf <- (v1 land v2) = 0;
		    m.sf <- (v1 land v2) <0;
  | CMP (a1,a2) -> let v2 = eval_arg m a2 in
		    let v1 = eval_arg m a1 in
		    m.zf <- (v1 = v2);
		    m.sf <- (v2 < v1);
  | PUSH a -> let v = eval_arg m a in
	      eval (SUB (C (IL 8), R RSP));
	      store_arg m v (ARI (IL 0,RSP));
  | POP a -> let v = eval_arg m (ARI (IL 0,RSP)) in
	     store_arg m v a;
	     eval (ADD (C (IL 8), R RSP));
  | RET -> eval (POP (R RIP));
  | JMP a -> let v = eval_arg m a in
	     store_arg m v (R RIP)
  | JZ a -> if m.zf then
      let v = eval_arg m a in
      store_arg m v (R RIP)
  | JNZ a -> if not m.zf then
      let v = eval_arg m a in
      store_arg m v (R RIP)
  | CALL a -> let v = eval_arg m a in
	       eval (PUSH (R RIP));
	       store_arg m v (R RIP);
  ...
  | LEAQ (a1,a2) -> let v = match a1 with
    | ARI(IL i,r) -> i+m.register.(reg_to_mem r)
    | AC(IL i) -> i
    | _ -> raise NotLinked in store_arg m v a2
  in
  eval ins
...
let run_machine level m =
  ...
  while get_reg m RIP !=  ((1 lsl 16) lor prog_bit) do
    if level >1 then (print_register m;
    print_stack m;
    print_ptr m);
