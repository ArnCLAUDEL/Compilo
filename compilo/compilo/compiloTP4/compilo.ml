...
let datasection = if isLinux then ASMType.LinkInstr "data"
...
let _ =
  ...
  let asm_prog = defListExt
    |> (fun x ->try List.fold_left (Generate.generate_asm_top varList) prog2 x
                with
...
(*    |> (fun x -> if !optim >0 then
	         ...
	         |> Optimisation.removeDeadCode
                 |> Optimisation.simplifyMov
	         else x)*)
    |> (fun x -> if !nocall then
                   List.filter (function | ASMType.CALL(_) -> false | _ -> true) x else x)
  in 
  if !doLinking then
